...
* 以下、列ごとの仕様を記述する。
  ...
  * 取引種別
    * 購入: 基軸暗号資産での購入
    * 売却: 基軸暗号資産または法定通貨での売却
    * 交換: 暗号資産同士の交換
    * 預入: 暗号資産の外部からの受入
    * 送付: 暗号資産の外部への送付
    * 入金: 法定通貨の預け入れ
    * 出金: 法定通貨の引き出し
    * 証拠金預入: 証拠金口座への法定通貨の移動
    * 証拠金引出: 証拠金口座からの法定通貨の移動
    * 受取』: 交換事業者からユーザーへの暗号資産または法定通貨の送付等
    * ハードフォーク: ハードフォークにより新規に入手した暗号資産の数量
    * 貸出: レンディングサービス利用時の暗号資産の貸出
    * 返済: レンディングサービス利用時の暗号資産の返済
    * 受取利息: レンディングサービス利用時の利息としての暗号資産の受取
    * その他: 上記に含まれない取引
  * 取引形態
